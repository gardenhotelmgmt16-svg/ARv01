  ...
  return (
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-md z-[100] flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
        {/* Compact Header */}
        <div className="bg-teal-600 px-6 py-6 text-white relative">
          <button 
            onClick={onClose}
            className="absolute top-4 right-4 w-8 h-8 bg-black/10 hover:bg-black/20 rounded-full flex items-center justify-center transition-colors text-lg"
            title="Close"
          >
            ✕
          </button>
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 bg-white/20 rounded text-[9px] font-black uppercase tracking-widest">Detail Tamu</span>
            <span className="font-mono text-[10px] opacity-70">#{booking.noBooking}</span>
          </div>
          <h2 className="text-2xl font-black tracking-tight leading-tight">{booking.guestName}</h2>
          <div className="flex items-center gap-1.5 mt-1 text-sm font-bold text-teal-50">
            <span>📱</span> {booking.phone}
          </div>
        </div>

        {/* Medium-Sized Content Layout */}
        <div className="p-6 space-y-6 bg-white overflow-y-auto max-h-[70vh]">
          {/* Section: Status & Duration */}
          <div className="grid grid-cols-3 gap-4">
            <div className="col-span-1 p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Room No</p>
               <p className="text-xl font-black text-slate-900">{booking.roomNumber}</p>
            </div>
            <div className="col-span-1 p-3 bg-teal-50 rounded-xl border border-teal-100 text-center">
               <p className="text-[9px] font-black text-teal-600 uppercase tracking-widest mb-0.5">Stay</p>
               <p className="text-xl font-black text-teal-900">{booking.nights} <span className="text-xs">Nts</span></p>
            </div>
            <div className="col-span-1 p-3 bg-slate-50 rounded-xl border border-slate-100 text-center">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">Pax</p>
               <p className="text-xl font-black text-slate-900">{booking.qtyPax}</p>
            </div>
          </div>

          {/* Section: Dates */}
          <div className="flex items-center gap-4 py-4 px-2 border-y border-slate-50">
          ...
          <div className="bg-slate-900 rounded-2xl p-5 text-white">
            <div className="flex justify-between items-center mb-4">
              <div>
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Rate/Night</p>
              ...
              <div className="text-right">
                ...
                <p className="text-xs font-bold uppercase">{booking.paymentMethod}</p>
              </div>
            </div>
            <div className="pt-3 border-t border-white/10 flex justify-between items-end">
              <div>
                <p className="text-[9px] font-black text-teal-400 uppercase tracking-widest mb-0.5">Grand Total</p>
                <p className="text-xl font-black">Rp {total.toLocaleString()}</p>
              </div>
              <span className="px-2 py-1 bg-white/10 rounded text-[9px] font-black uppercase tracking-tighter">
          ...
          <div className="space-y-2">
            <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Additional Notes</h4>
        ...
        {/* Visual Footer with primary Close button */}
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex flex-col gap-2">
           <div className="flex justify-between text-[10px] font-bold text-slate-400 px-2">
             <span>Reserved By: {booking.reservationBy || "-"}</span>
             <span>Ref: {booking.id.toUpperCase().slice(0, 8)}</span>
           </div>
           <button 
             onClick={onClose}
             className="w-full py-3 bg-slate-900 text-white font-black rounded-xl hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/10 text-sm"
           >
             Close View
           </button>
        </div>
...
const App: React.FC = () => {
  ...
  const roomStatusMap = useMemo(() => {
    ...
    // Rentang pencarian dengan validasi "Between"
    let searchStart = roomStatusSearchIn || todayStr;
    let searchEnd = roomStatusSearchOut || searchStart;

    // Jika user input tanggal terbalik, otomatis swap agar logika Between jalan
    ...
    // Tentukan "Besok" relatif terhadap searchStart untuk status Reserved
    ...
    bookings.forEach(b => {
      // Logika Occupied: Booking Check-In < Range Check-Out DAN Booking Check-Out > Range Check-In
      // Ini adalah standar industri untuk irisan ketersediaan (Exclusive overlapping)
      if (b.checkInDate < searchEnd && b.checkOutDate > searchStart) {
        if (statuses[b.roomNumber]) statuses[b.roomNumber].occupied = true;
      }
      
      // Logika Reserved: Unit yang akan di Check-In besok hari dari tanggal pencarian
  ...
  const exportToExcel = () => {
    ...
    XLSX.writeFile(workbook, `EmeraldHMS_Bookings_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const Sidebar = () => (
    <div className="w-64 bg-slate-900 text-slate-300 h-screen sticky top-0 flex flex-col p-4 z-20 shadow-2xl">
      <div className="flex items-center gap-3 px-2 py-6 mb-8 border-b border-slate-800">
        <div className="w-10 h-10 bg-teal-500 rounded-xl flex items-center justify-center text-white font-black text-xl shadow-lg shadow-teal-500/20">E</div>
        <h1 className="text-xl font-black text-white tracking-tighter">EMERALD <span className="text-teal-500">HMS</span></h1>
      </div>
  ...
  const PublicView = () => (
    <div className="min-h-screen p-8 max-w-7xl mx-auto">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          <h1 className="text-4xl font-black text-slate-800 tracking-tight mb-2">Emerald <span className="text-teal-600">Hotel</span></h1>
